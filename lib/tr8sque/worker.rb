module Tr8sque
  # A Tr8sque Worker processes jobs. On platforms that support fork(2),
  ...
  class Worker
    include Tr8sque::Helpers
    extend Tr8sque::Helpers
    ...
    def queues
      @queues.map {|queue| queue == "*" ? Tr8sque.queues.sort : queue }.flatten.uniq
    end
    ...
    # control, the Tr8sque workers will not die gracefully and therefore
    ...
    def run_hook(name, *args)
      return unless hook = Tr8sque.send(name)
    ...
    # Find Tr8sque worker pids on Linux and OS X.
    #
    # Returns an Array of string pids of all the other workers on this
    # machine. Useful when pruning dead workers on startup.
    ...
    # Find Tr8sque worker pids on Solaris.
    #
    # Returns an Array of string pids of all the other workers on this
    # machine. Useful when pruning dead workers on startup.
    def solaris_worker_pids
      `ps -A -o pid,comm | grep "[r]uby" | grep -v "resque-web"`.split("\n").map do |line|
        ...
        if pargs_command.split(':')[1] == " resque-#{Tr8sque::Version}"
          real_pid
        end
      end.compact
    end
    ...
    def procline(string)
      $0 = "resque-#{Tr8sque::Version}: #{string}"
      log! $0
    end
