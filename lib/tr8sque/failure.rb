module Tr8sque
  ...
  # backend is being used. For instance, the Tr8sque web app uses it to display
  ...
  module Failure
    ...
    # `Tr8sque::Failure::Base`.
    #
    # Example use:
    #   require 'tr8sque/failure/hoptoad'
    #   Tr8sque::Failure.backend = Tr8sque::Failure::Hoptoad
    def self.backend=(backend)
      @backend = backend
    end

    # Returns the current backend class. If none has been set, falls
    # back to `Tr8sque::Failure::Redis`
    def self.backend
      return @backend if @backend
      require 'tr8sque/failure/redis'
    ...
    def self.requeue_queue(queue)
      i=0
      while job = Tr8sque::Failure.all(i)
        if job['queue'] == queue
          Tr8sque::Failure.requeue(i)
        end  
        i+=1
      end
    end
