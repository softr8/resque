module Tr8sque
  # A Tr8sque::Job represents a unit of work. Each job lives on a
  ...
  #
  #   job = Tr8sque::Job.reserve(:high)
  #   klass = Tr8sque::Job.constantize(job.payload['class'])
  #   klass.perform(*job.payload['args'])
  class Job
    include Helpers
    extend Helpers

    # Raise Tr8sque::Job::DontPerform from a before_perform hook to
    # abort the job.
    ...
    def self.create(queue, klass, *args)
      Tr8sque.validate(klass, queue)

      if Tr8sque.inline?
        constantize(klass).perform(*decode(encode(args)))
      else
        Tr8sque.push(queue, :class => klass.to_s, :args => args)
      end
    end
    ...
    #
    #   Tr8sque::Job.destroy(queue, 'UpdateGraph')
    #
    # Whereas specifying args will only remove the 2nd job:
    #
    #   Tr8sque::Job.destroy(queue, 'UpdateGraph', 'mojombo')
    #
    ...
    # Given a string queue name, returns an instance of Tr8sque::Job
    # if any jobs are available. If not, returns nil.
    def self.reserve(queue)
      return unless payload = Tr8sque.pop(queue)
      new(queue, payload)
    end
